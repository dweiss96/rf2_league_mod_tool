use crate::models::{Config, VersionInfo, VersionSpec};
use std::fs;
use std::path::Path;
use std::sync::mpsc::Sender;

automod::dir!("src/tasks");

#[cfg(test)]
mod tests {
    use super::*;
    #[test]
    fn it_reads_config() {
        let cfg = read_config("tests/example_files/config.json");
        assert_eq!(cfg.league.name, "Test Liga 23 S1")
    }
}

pub fn read_config(cfg_path: &str) -> Config {
    let path = Path::new(cfg_path);
    let file = fs::read(path);
    let content = String::from_utf8(file.unwrap()).unwrap();

    serde_json::from_str(content.as_str()).unwrap()
}

pub fn find_base_version(car_ws_id: String, ws_path: &Path) -> VersionSpec {
    let files = fs::read_dir(ws_path.join(car_ws_id)).unwrap();
    let base_version = files
        .map(|r| r.unwrap())
        .map(|f| {
            if f.file_name().to_str().unwrap().ends_with(".rfcmp") {
                let file_name = f.file_name();
                let version_name = file_name
                    .to_str()
                    .unwrap()
                    .split_once("v")
                    .unwrap()
                    .1
                    .trim_end_matches(".rfcmp");
                let vc = VersionSpec::decode_from_str(version_name);
                if vc.minor % 2 == 0 {
                    vc
                } else {
                    VersionSpec::empty()
                }
            } else {
                VersionSpec::empty()
            }
        })
        .find(|v| v.major > 0);

    base_version.unwrap_or(VersionSpec::empty())
}

pub fn generate(
    config: Config,
    tmp_dir: &str,
    target_dir: &str,
    version: &str,
    tx: Sender<String>,
) {
    // TODO: Create logos on the fly

    // copy all files to tmp working directory
    copy_files::copy(
        std::env::current_dir().unwrap().to_str().unwrap(),
        tmp_dir,
        config.league.clone(),
    );

    for car in config.league.clone().cars {
        let version_info = VersionInfo {
            base_version: car
                .clone()
                .version_overwrite
                .map(VersionSpec::decode_from_string)
                .unwrap_or_else(|| find_base_version(
                    car.clone().workshop_id,
                    Path::new(&config.paths.workshop),
                )),
            modpack_version: VersionSpec::decode_from_str(version),
        };
        let _ = rfactor2::write_cmp_info(tmp_dir, version_info, config.league.clone(), car.clone());
        rfactor2::fill_mas_files(
            tmp_dir,
            config.paths.modmgr.as_str(),
            config.league.clone(),
            car.clone(),
            tx.clone(),
        );
        rfactor2::generate_rfcmp_files(
            tmp_dir,
            config.paths.modmgr.as_str(),
            car.clone(),
            tx.clone(),
        );
    }

    // copy results to target dir
    fs::read_dir(tmp_dir).unwrap().for_each(|er| {
        let e = er.unwrap();
        if e.file_type().unwrap().is_dir() {
            fs::read_dir(e.path()).unwrap().for_each(|entry| {
                let file_name = entry.unwrap().file_name();
                let file_name_string = file_name.to_str().unwrap();
                if file_name_string.ends_with(".rfcmp") {
                    let _ = fs::copy(
                        e.path().join(file_name_string),
                        Path::new(target_dir).join(file_name_string),
                    )
                    .unwrap();
                }
            });
        } else {
            let file_name = e.file_name();
            let file_name_string = file_name.to_str().unwrap();
            if file_name_string.ends_with(".rfcmp") {
                let _ = fs::copy(
                    e.path(),
                    Path::new(target_dir).join(file_name_string),
                )
                    .unwrap();
            }
        }
    });
}
