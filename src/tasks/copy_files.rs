    ...
    fn get_league_cars_and_drivers() -> (League, Vec<Car>, Vec<Driver>) {
        ...
        let cars = vec![
            Car {
                ...
                version_overwrite: "1.00".to_string()
            },
            Car {
        ...
        let league = League {
            ...
            livery_file_prefix: String::from(""),
            livery_file_suffix: String::from("TST"),
            version_prefix: String::from("TST23"),
            car_files: vec![
                "TestLigaUpgrades.ini".to_string(),
                "brand_logo.png".to_string()
            ],
            league_files: vec!["CAR_GT3_2023_v1.00.rfcmp".to_string()],
            cars: cars.clone(),
...
pub fn copy(cfg_dir: &str, tmp_dir: &str, league: League, driver: Driver) {
    copy_league_files(cfg_dir, tmp_dir);
    league.cars.iter().for_each(|car| {
        copy_car_files(cfg_dir, tmp_dir, car.id.as_str());
        copy_driver_files(cfg_dir, tmp_dir, league.clone(), driver.clone());
    });
}
...
fn copy_driver_files(cfg_dir: &str, tmp_dir: &str, league: League, driver: Driver) {
    ...
    let _ = fs::write(out_dir.join(skin_file_name.clone()), updated_veh);
